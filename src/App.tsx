...
const App = () => {
  ...
  useEffect(() => {
    if (query) {
      vscodeApi.postMessage({
        command: "query",
        value: query,
        library: JSON.stringify(selectedLibrary),
      });
    }
  }, [debouncedQuery]);
      ...
      switch (message.data.command) {
        case "recentlyUsedDocs":
          ...
          const sortedLibraries = librariesFromFile.sort((a, b) => {
            const aIndex =
              recentlyUsedDocs.indexOf(a.indexName) === -1
                ? 1000
                : recentlyUsedDocs.indexOf(a.indexName);
            const bIndex =
              recentlyUsedDocs.indexOf(b.indexName) === -1
                ? 1000
                : recentlyUsedDocs.indexOf(b.indexName);
  ...
  const handleSetIframeUrl = (url: string) => {
    if (selectedLibrary) {
      vscodeApi.postMessage({
        command: "openDocs",
        libraryIndexName: selectedLibrary.indexName,
          ...
          return (
            <div
              ...
              className={cn("item-container", {
                "item-container-hover": cursor === item.id,
              })}
              ...
              <div>
                <span
                  className={cn("icon-container", {
                    "icon-container-hover": cursor === item.id,
                  })}
                >
                ...
                <span
                  ...
                  dangerouslySetInnerHTML={{ __html: item.highlightResult }}
                />
              </div>
      ...
      case 2:
        if (item.isHighestLvl) {
          return (
            <div
              onClick={() => handleSetIframeUrl(item.url)}
              className={cn("item-container lvl2-and-lvl3", {
                "item-container-hover": cursor === item.id,
              })}
              onMouseEnter={() => setHovered(item.id)}
              onMouseLeave={() => setHovered(undefined)}
          ...
          <div className="inner-container">
            <div className="select-container">
              ...
              <Select
                ...
                onChange={(value) => {
                  setSelectedLibrary(
                    libraries.find((lib) => lib.displayName === value)
                  );
                  setTimeout(() => {
